...
public class HttpServer {
            ...
            if (uri.contains("audio")) {
                try {
                    ...
                    if (start <= end) {
                        ...
                        // Attempt to skip to the specified start byte
                        long skippedBytes = audioInputStream.skip(start);
                        if (skippedBytes != start) {
                            // Handle the case where the number of actually skipped bytes doesn't match the expected start
                            throw new IOException("Skipped " + skippedBytes + " bytes, but was attempting to skip " + start + " bytes.");
                        }
    ...
    static {
        ...
        MIME_TYPES.put("java", "text/x-java-source, text/java");
            MIME_TYPES.put("md", MimeTypes.TEXT_PLAIN);
    MIME_TYPES.put("txt", MimeTypes.TEXT_PLAIN);
    MIME_TYPES.put("asc", MimeTypes.TEXT_PLAIN);
    ...
    public static String getMimeType(String extension) {
        return MIME_TYPES.getOrDefault(extension, TEXT_PLAIN);
    }
}